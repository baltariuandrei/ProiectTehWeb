...
// get request => a json array with all the students
...
// get request to /students/:sid => response: a json with a student by id
...
// post requests to /students/add => add a student
...
// get request to /students/:sid/:aid/feedbacks => a json array with the feedback that a student has on a specific activity
router.get('/students/:sid/:aid/feedbacks', async(req, res, next) => {
    try {
       let student = await models.Student.findByPk(req.params.sid, {include : [models.Feedback]});
       let activityFeedback = [];
       if (student) 
       {
           student.feedback.forEach(fdb => {
               if (fdb.idActivity == req.params.aid) 
               {
                activityFeedback.push(fdb)
               }
           })
           res.status(200).json(activityFeedback)
       } else 
       {
...
// get request to /students/:sid/feedback => a json array with all the feedback of a student
router.get('/students/:sid/feedback', async(req, res, next) => {
   try {
       let student = await models.Student.findByPk(req.params.sid, {include : [models.Feedback]});
...
// delete request to /students/:id => delete a student by id
